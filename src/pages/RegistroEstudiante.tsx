  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await authService.register(
        ...
        formData.nombre,
        formData.apellido,
        'olimpista' // Rol fijo para estudiantes
      );
      ...
      if (result.success && result.data?.user && result.data?.token) {
        ...
        // Redirigir a la vista de olimpista
        setTimeout(() => {
          navigate('/olimpista');
        }, 2000);
      } else {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 md:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {successMessage && (
              <div className="bg-green-500/20 border border-green-500/50 rounded-lg p-3 text-green-700 text-sm">
                {successMessage}
              </div>
            )}
            
            {errors.general && (
              <div className="bg-red-500/20 border border-red-500/50 rounded-lg p-3 text-red-700 text-sm">
                {errors.general}
              </div>
            )}
            ...
            <div className="text-center">
              <button
                type="submit"
                disabled={loading}
                className="bg-primary-500 hover:bg-primary-600 disabled:bg-gray-400 text-white px-8 py-3 rounded-md font-medium flex items-center space-x-2 mx-auto transition-colors"
              >
                {loading ? (
                  <>
