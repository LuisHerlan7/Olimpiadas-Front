...
import { getFaseInscripcion, updateFaseInscripcion, cancelarFaseInscripcion, type FaseInscripcion } from "../../../services/fases";
...
export default function FasesPage() {
  ...
  const handleCancelarFase = async () => {
    if (!confirm("¿Estás seguro de cancelar la fase de inscripción? Esto bloqueará la subida de inscritos.")) {
      return;
    }
  ...
  const verificarSiEstaEnFecha = (): boolean => {
    if (!faseInscripcion?.fecha_inicio || !faseInscripcion?.fecha_fin) {
      return false;
    }
    const ahora = new Date();
    const inicio = new Date(faseInscripcion.fecha_inicio);
    const fin = new Date(faseInscripcion.fecha_fin);
    ...
    >
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                ...
                <div className={`rounded-xl border p-4 ${
                  faseInscripcion?.cancelada 
                    ? "border-red-500/50 bg-red-500/10" 
                    : verificarSiEstaEnFecha()
                  ...
                  <p className="text-sm text-slate-300">
                    {faseInscripcion?.mensaje || (verificarSiEstaEnFecha() 
                    ...
                    <div className="space-y-2">
                      <div>
                        ...
                        <p className="text-lg font-semibold text-white">
                          {faseInscripcion?.fecha_inicio 
                            ? new Date(faseInscripcion.fecha_inicio).toLocaleDateString('es-BO', { 
                                year: 'numeric', 
                                month: 'long', 
                                day: 'numeric' 
                              })
                      ...
                      <div>
                        ...
                        <p className="text-lg font-semibold text-white">
                          {faseInscripcion?.fecha_fin 
                            ? new Date(faseInscripcion.fecha_fin).toLocaleDateString('es-BO', { 
                                year: 'numeric', 
                                month: 'long', 
                                day: 'numeric' 
                              })
                    ...
                    {editando ? (
                      ...
                      <>
                        ...
                        <button
                          ...
                          disabled={guardando || faseInscripcion?.cancelada}
      ...
      {activeFase === "asignacion" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-3xl rounded-2xl border border-white/10 bg-slate-900/95 p-6 shadow-2xl backdrop-blur-xl">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-2xl font-bold text-white">Fase: Asignación</h2>
              <button
                onClick={cerrarFase}
                className="text-slate-300 hover:text-white transition-colors"
              >
            ...
            <div className="rounded-xl border border-white/10 bg-slate-800/50 p-6">
              <p className="text-slate-300">Contenido de la fase de Asignación.</p>
              <p className="text-sm text-slate-400 mt-2">Aquí puedes asignar evaluadores y responsables a los participantes.</p>
            </div>
          </div>
        </div>
      )}
